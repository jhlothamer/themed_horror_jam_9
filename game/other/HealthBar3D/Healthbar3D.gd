tool
extends Spatial
class_name healthbar3D, "res://other/HealthBar3D/class_icon.png"
...
func value_set(val):
	...
	#progress_origin.transform.origin.x = size.x * (1.0 -value) * .5
	var new_x = size.x * value
	var diff_x_size = size.x - new_x
	progress_origin.transform.origin.x = diff_x_size * .5
	progress_origin.transform.origin.z = -.01
